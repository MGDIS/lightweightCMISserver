...
public class ObjectStoreImpl implements ObjectStore {
    ...
    public ObjectStoreImpl(String repositoryId, String filePath, IPersistenceManager persistenceManager) {
    ...
    public String storeObject(StoredObject so, boolean saveOnExit) {
        ...
        if (null == id) {
            File newFile = persistenceManager.calculateFile(fStoredObjectMap, so);
            if (newFile != null) {
    ...
    public boolean isTypeInUse(String typeId) {
        ...
        for (String objectId : getIds()) {
            ...
            if (so.getTypeId().equals(typeId) || (so.getSecondaryTypeIds() != null && so.getSecondaryTypeIds().contains(typeId))) {
                return true;
            }
        }
        return false;
